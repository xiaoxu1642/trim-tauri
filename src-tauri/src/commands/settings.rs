...
/// IPv6 侧私有判定。除逐网段比对外，**6to4 / Teredo / NAT64 一律按私有处理**：前两者把 IPv4
/// 地址编码进自身（`2002:v4::`、`2001:0::v4`），NAT64 把内嵌 IPv4 挂在 `64:ff9b::/96` 之后，
/// 都是隧道式绕过本机字面量校验的现成通道，而没有任何一家模型服务会用这类字面量当 API 端点。
fn ipv6_private(b: &[u8; 16]) -> bool {
    ...
    if b[0] == 0x00 && b[1] == 0x64 && b[2] == 0xff && b[3] == 0x9b {
        // NAT64 64:ff9b::/32（含知名前缀 /96 与本地用 /48）：内嵌 IPv4 可经网关直达内网，按私有拒。
        // 注意每组是 16 位：`64:ff9b` 展开成字节是 00 64 ff 9b，首组 `64` 是 0x0064 而非单字节 0x64。
        return true;
    }
    false
}
...
mod tests_private_url {
    ...
    fn ipv6_mapped_and_tunnel_forms_are_private() {
        for u in [
            ...
            "http://[64:ff9b::a00:1]/",           // NAT64 知名前缀内嵌 10.0.0.1
            "http://[64:ff9b::10.0.0.1]/",        // NAT64 内嵌 IPv4 的点分形式
