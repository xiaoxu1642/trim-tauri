# <<<PROVENANCE
# 来源：C:/KaiFa/Trim/src/scripts-powershell/runtimes-scripts.js → repair("netfx48", "C:/KaiFa/Trim/src/scripts-powershell/runtimes-scripts.js")
# 生成：tools/sync-ps-from-js.mjs 直接取 JS **运行时字符串值**写入，无任何字符替换；
#       改动本文件必须在源仓库改 JS 后重跑生成器（校验见 tools/check-ps-extraction.mjs）。
# 说明：静默执行本地安装包：.NET Framework 4.8（哨兵模板：安装包路径）
# PROVENANCE>>>
...
Write-Output '正在安装 .NET Framework 4.8（静默模式，请稍候）...'
$result = Start-Process -FilePath 'C:/KaiFa/Trim/src/scripts-powershell/runtimes-scripts.js' -ArgumentList '/q /norestart' -Wait -PassThru
