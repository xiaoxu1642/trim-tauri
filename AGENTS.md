...
由 Electron 版逐域迁移而来，迁移进度与批次史实见仓库内方案文档。
...
2. **命令内显式来源校验** —— `engine::guard::guard(&window, guard::MAIN)`（高危、主窗专属）或 `guard_readonly`（只读，含子窗口 label）。多窗口应用**不能裸注册命令**：子窗口一旦被注入，裸注册就让它能调主窗专属高危通道。
...
- 发布前另须实跑被 `#[ignore]` 的门禁用例（真实 pwsh / 网络 / 大目录 / DPAPI 密文样本；数量与命令以仓库内门禁文档为准）。
...
12. 后台调试不要试图用 CDP 端口验证窗口 OS 手感（拖动/缩放是 `WM_NCLBUTTONDOWN` 模态循环，合成事件测不出来）。
